...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        ...
        try (PreparedStatement updQuery = connection.prepareStatement(Queries.UPDATE_MESSAGE)) {
            ...
            updQuery.setLong(3, Objects.requireNonNull(findUserById(message.getAuthor().getId())).getId());
            updQuery.setLong(4, (Objects.requireNonNull(findChatroomById(message.getRoom().getId())).getId()));
            updQuery.setLong(5, message.getId());
